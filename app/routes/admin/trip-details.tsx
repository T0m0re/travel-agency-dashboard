        ...
        <section className="container wrapper-md">
            ...
            <section className="gallery">
                {imageUrls.map((url: string, i : number) => (
                    <img
                        src={url} key={i} className={cn('w-full rounded-xl object-cover', i === 0 ? 'md:col-span-2 md:row-span-2 h-[330px]' : "md:row-span-1 h-[150px]")}
                    />
                ))}
            </section>
        ...
        <section className="flex flex-col gap-6">
                ...
                <div className="trip-grid">
                    {allTrips.map(({id, name, imageUrls, itinerary, interests, travelStyle, estimatedPrice}) => (
                        <TripCard id={id} key={id} name={name} location={itinerary?.[0].location ?? ""} imageUrl={imageUrls[0]} tags={[interests, travelStyle]} price={estimatedPrice}/>
                    ))}
                </div>
            </section>
    </main>
